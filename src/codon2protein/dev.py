...
def read_fasta(fasta_file):
    ...
    fasta_file_path = os.path.join(os.path.expanduser('~'), 
                                   'codon2protein','src','codon2protein',str(fasta_file))
...
find_start_codon(fasta_string=read_fasta('human_notch.fasta'), first=False, oof=True)
...
read_codon_table('~/codon2protein/src/codon2protein/codon_table.csv')
...
def translate(fasta_file, codon_table):
    ...
    for start_codon in codon_starts:
        ...
        for i in range(start_codon, len(sequence), 3):
            ...
            amino_acid = read_codon_table(codon_table).get
